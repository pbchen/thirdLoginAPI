...
class Media_QQModel {
    ...
    public function __construct($basedb,$qqparam=array()) {
        $this->_qqparam=array_merge(array('media_id'=>13),$qqparam);
        $this->_db = $basedb->_db;
    ...
    public function getApp($app_id){
        ...
        $table = '`app`';
        $where = array("`app`.`app_id`='{$app_id}'", "`app_media`.`media_id`=" . $this->_qqparam['media_id']);
    ...
    public function saveUser($openid,$qqUser,$APPID){
        $result=FALSE;
        $media_qq_user_id = $this->_basemodel->get(array('id'),'media_qq_user',array("openid='{$openid}'","APPID='{$APPID}'"));
        if(!$media_qq_user_id){
            $media_qq_user_id = $this->_basemodel->insert('media_qq_user',array('openid'=>$openid,'APPID'=>$APPID),TRUE);
        }else{
            $media_qq_user_id = $media_qq_user_id['id'];
        }
        if($media_qq_user_id){
            $data=array(
                'mediaUserID'=> 10000000000 * (int)$this->_qqparam['media_id'] + $media_qq_user_id,
                'mediaID'=>(int)$this->_qqparam['media_id'],
                'screenName'=>$qqUser->nickname,
                'profileImageUrl'=>$qqUser->figureurl_2
            );
//            print_r($data);
    ...
    public function saveAccessToken($app_id,$access_token,$openid,$media_qq_user_id,$usertokentime){
        $result=FALSE;
        $nowT=time();
        $now=  date('Y-m-d H-i-s',$nowT);
        $e_time = $usertokentime;
        $login_token = md5($now.  uniqid().$access_token.$openid);
        $data= array(
            'c_time'=>$now,
...
//        print_r($data);
        ...
        $this->_basemodel->delete('login_media_qq_token',array("qq_openid='{$openid}'"));
        if($this->_basemodel->insert('login_media_qq_token',$data)){
            $login_token_data = array(
                ...
                'app_id'=>  $app_id,
                'token'=>$login_token,
                'media_id'=>(int)$this->_qqparam['media_id'],
